...
BVDataPTAImpl::BVDataPTAImpl(PointerAnalysis::PTATY type) :
	...
	} else if (type == FSSPARSE_WPA || type == FSTBHC_WPA || type == FSCS_WPA) {
		if (INCDFPTData)
 ...
 */
void PointerAnalysis::validateSuccessTests(const std::string fun) {
 ...
 */
void PointerAnalysis::validateExpectedFailureTests(const std::string fun) {
...
AliasResult BVDataPTAImpl::alias(const PointsTo& p1, const PointsTo& p2) {
    ...
    else if (pts1.intersects(pts2) && pts2.intersects(pts1) && pts1.count() == 1 && pts2.count() == 1)
        // Under-approximation of must-alias relations.
        return llvm::MustAlias;
    else if (pts1.intersects(pts2))
        return llvm::MayAlias;
    else
        return llvm::NoAlias;
}
